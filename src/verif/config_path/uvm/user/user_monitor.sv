class user_monitor extends uvm_monitor;
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db #(virtual user_config_if) :: get(this, "", "vif", vif))
            `uvm_fatal("NOVIF", "user_monitor: virtual interface 'vif' not set via uvm_config_db")

        if (!uvm_config_db #(virtual clk_rst_if)          :: get(this, "", "clk_vif", clk_vif))
            `uvm_fatal("NOVIF", "user_monitor: virtual interface 'clk_vif' not set via uvm_config_db")
    endfunction
    ...
    task collect_write_req();
        ...
        `uvm_info("USER_MON", $sformatf("Collected %s", item.convert2string()), UVM_HIGH)
        req_ap.write(item);
    endtask
    ...
    task collect_resp(input bit is_write);
        ...
        item = user_resp_item::type_id::create("item");
